...
public class AdminSevrlet extends HttpServlet {
	...
	private void sereachInfo(HttpServletRequest request,
		...
		int payforType=Integer.parseInt(request.getParameter("payforType")==null?"-1":request.getParameter("payforType"));
		//获取stateType，若为空则默认为-1
		int stateType=Integer.parseInt(request.getParameter("stateType")==null?"-1":request.getParameter("stateType"));
		//获取showType.infoType，若为空则默认为0；
		int infoType=Integer.parseInt(request.getParameter("showType.infoType")==null?"0":request.getParameter("showType.infoType"));
